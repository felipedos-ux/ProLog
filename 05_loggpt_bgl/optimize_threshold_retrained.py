"""
Script de Otimização de Threshold para Recall (Retrained Model).
...
# CHANGED: Import from config_retrain
...
def prepare_window(indices, templates, labels, tokenizer, max_context):
    ...
    for i, current_log in enumerate(w_templates):
        ...
        # FIX: Clamp target_start_idx to 0 to avoid negative indexing
        if target_start_idx < 0: target_start_idx = 0
...
def main():
    logger.info("🔍 Threshold Optimization for High Recall (RETRAINED)")
    ...
    if os.path.exists(f"{MODEL_DIR}/loggpt_weights.pt"):
    ...
    # Load ORIGINAL Data (to maintain same validation set logic)
    # config_retrain DATA_FILE points to BGL_windowed_train.csv
    # We should use BGL_processed.csv for consistency with detect_windowed_validation.py logic
    # BUT, to keep it simple, let's just stick to what config_retrain says for consistency with training?
    # NO. Validation must happen on RAW sequence of events to simulate real life (and detect_windowed_validation used raw).
    # So we force override DATA_FILE to raw BGL if needed.
    
    RAW_DATA = "D:/ProLog/data/BGL_processed.csv"
    logger.info(f"Loading raw data from {RAW_DATA}")
    df = pl.read_csv(RAW_DATA, infer_schema_length=INFER_SCHEMA_LENGTH)
    ...
    # Increase sample size slightly for better robust stats
    ...
    for r in results:
        ...
        if r['threshold'] % 2 == 0 or r['recall'] > 0.8: # Show more granularity
